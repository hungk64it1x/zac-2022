...
class Model():
    ...
    def __init__(self, name='convnext_large', checkpoint_model='', model_prefix='', size=224):
...
def get_args_parser():
    ...
    parser.add_argument('--num_frame_pred', type=int, default=3, help='predict.')
    parser.add_argument('--data_predict', default='datasets/image_cls/imagenet1k/', type=str, help='dataset predict path')
    parser.add_argument('--predict', action='store_true', help='predict.')
    parser.add_argument('--sample_submission_path', type=str, help='path to dataset')
...
def main(args):
    ...
    if args.predict:
        ...
        tfms_hflip = transforms.Compose([transforms.RandomHorizontalFlip(p=1),
                                        transforms.Resize(size), 
                                        transforms.ToTensor(),
                                        transforms.Normalize(IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)])

        tfms_vflip = transforms.Compose([transforms.RandomVerticalFlip(p=1),
                                        transforms.Resize(size), 
                                        transforms.ToTensor(),
                                        transforms.Normalize(IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)])
            
        outputs = []
        for image_id in tqdm(submit_df['fname'].values.tolist()):
            id = image_id.split('.')[0]
            model.eval()
            avg_frame = []
            for i in range(args.num_frame_pred):
                if args.TTA:
                    output_logits = []

                    img1 = tfms(Image.open(os.path.join(args.data_predict, f'{id}/{id}_{i}.jpg'))).unsqueeze(0).cuda()
                    img2 = tfms(Image.open(os.path.join(args.data_predict, f'{id}/{id}_{i}.jpg'))).unsqueeze(0).cuda()
                    img3 = tfms(Image.open(os.path.join(args.data_predict, f'{id}/{id}_{i}.jpg'))).unsqueeze(0).cuda()

                    with torch.no_grad():
                        output1 = model(img1)
                        output2 = model(img2)
                        output3 = model(img3)
                        output_logits.append(output1)
                        output_logits.append(output2)
                        output_logits.append(output3)
                    output_logits = torch.cat(output_logits)

                    output = torch.sum(output_logits, dim=0, keepdim=True) / 3
                    
                    output = output.softmax(1).to('cpu').numpy()
                    # print(output)
                    
                else:
                    img = tfms(Image.open(os.path.join(args.data_predict, f'{id}/{id}_{i}.jpg'))).unsqueeze(0).cuda()
                    with torch.no_grad():
                        output = model(img)
                    output = output.softmax(1).to('cpu').numpy()
                    
                avg_frame.append(output)
            avg_frame = np.concatenate(avg_frame) 
            avg_frame = np.mean(avg_frame, axis=0, keepdims=True)
            outputs.append(avg_frame)

        outputs = np.concatenate(outputs)
        # print(outputs.tolist())
        liveness_score = outputs[:, 1]
        # liveness_score[liveness_score >= 0.4] = 1
        # liveness_score[liveness_score < 0.4] = 0
        submit_df['liveness_score'] = liveness_score

        submit_df.to_csv(f'./submission/{args.name}.csv', index=False)
        return 
